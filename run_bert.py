...
# 损失函数导入（4标签）
from pybert.train.losses import CrossEntropy, MultiLabelCrossEntropy, BCEWithLoss, \
...
# from pybert.model.bert_for_multi_label_textcnn import BertForMultiLable
# from pybert.model.bert_for_multi_label_kg import BertForMultiLable
# from pybert.model.bert_for_multi_label_prompt import BertForMultiLable
# from pybert.model.bert_for_multi_label_live2 import BertForMultiLable
# from pybert.model.bert_for_multi_label_meta import BertForMultiLable
#from pybert.model.bert_for_multi_label_kg_temp import BertForMultiLable
...
def check_model_parameters(model):
    ...
    # 1. BERT 参数
    bert_params = dict(model.bert.named_parameters())
    print(f"  BERT 参数数量: {len(bert_params)}")
    # print(f"  BERT 第一层权重 norm: {torch.norm(next(iter(bert_params.values())))}")

    # 2. GCN 参数
    gcn_params = dict(model.gcn.named_parameters())
    print(f"  GCN 参数数量: {len(gcn_params)}")
    # print(f"  GCN 第一层权重 norm: {torch.norm(next(iter(gcn_params.values())))}")

    # 3. 分类头
    print(f"  分类头权重 norm: {torch.norm(model.classifier.weight)}")

    # 4. Buffers
    # print(f"  node_features shape: {model.node_features.shape}")
    # print(f"  edge_weights shape: {model.edge_weights.shape}")
    # print(f"  node_features device: {model.node_features.device}")
    # print(f"  alpha: {model.alpha}")

    # 5. 是否所有参数都在 CUDA 上
    # all_on_gpu = all(p.is_cuda for p in model.parameters())
    # print(f"  所有参数是否在 GPU: {all_on_gpu}")
...
def run_train(args):
    ...
    train_monitor = TrainingMonitor(file_dir=config['figure_dir'], arch=args.arch, time=args.time)
    # 在训练过程中保存模型权重的。这里传入了以下参数
    # 每个 Epoch 评估一次，若为最佳模型则保存
...
def main():
    ...
    # 时间戳
    parser.add_argument('--time', type=str, default='999912312400')

    # Knowledge Graph
    # GCN & 知识图谱参数
    parser.add_argument("--node_features_path", type=str, default='', help='节点特征放哪儿了?')
    parser.add_argument("--adj_matrix_path", type=str, default='', help='邻接矩阵放哪儿了？')
    ...
    # 设置检查点目录:
    # 只有当模型训练时，添加 checkpoint_dir 时间后缀
    if args.do_train and not args.resume_path:
        config['checkpoint_dir'] = config['checkpoint_dir'] / timestamp
