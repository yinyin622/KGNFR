...
class HammingScore(Metric):
    ...
    def __call__(self, logits, target):
        ...
        if self.search_thresh:
            thresh, score = self.thresh_search(y_prob)
            return score
        else:
    ...
    def thresh_search(self, y_prob):
        ...
        for threshold in np.arange(0.01, 1.0, 0.01):
            ...
            # ✅ 正确的 Hamming Score = 1 - hamming_loss
            score = 1.0 - hamming_loss(self.y_true, y_pred)
            if score > best_score:
                best_threshold = threshold
                best_score = score
        return best_threshold, best_score
    ...
    def value(self):
        return 1.0 - hamming_loss(self.y_true, self.y_pred)
...
class ClassReport(Metric):
    ...
    def __init__(self):
        ...
        self.target_names = ['Usa', 'Sup', 'Rel', 'Per']
...
class MultiLabelReport(Metric):
    ...
    # def value(self):
    #     """
    #     恢复原始逻辑：对每个标签独立搜索最优阈值（按 F1 最大化），
    #     并报告该标签在最优阈值下的 AUC, P, R, F1, Hamming Loss 等。
    #     """
    #     results = []
    ...
    #         # Step 1: 搜索该标签的最优阈值（按 F1 最大）
    #         for th in np.arange(0.01, 1.0, 0.01):
    #             y_pred_i = (self.y_prob[:, i] > th).astype(int)
    #             # 注意：这里 average='binary'，因为我们只看当前标签
    #             f1 = f1_score(self.y_true[:, i], y_pred_i, average='binary', zero_division=0)
    #             if f1 > best_f1:
    #                 best_f1 = f1
    #                 best_thresh = th
    #                 best_p = precision_score(self.y_true[:, i], y_pred_i, average='binary', zero_division=0)
    #                 best_r = recall_score(self.y_true[:, i], y_pred_i, average='binary', zero_division=0)

    #         # Step 2: 计算 AUC（不需要阈值）
    #         try:
    #             auc = roc_auc_score(self.y_true[:, i], self.y_prob[:, i])
    #         except ValueError:
    #             # 如果全是 0 或全是 1，AUC 无法计算
    #             auc = float('nan')

    #         # Step 3: 计算 Hamming Loss（仅针对该标签）
    #         y_pred_best = (self.y_prob[:, i] > best_thresh).astype(int)
    #         h_loss = hamming_loss(self.y_true[:, i], y_pred_best)
    ...
    #     # 可选：计算全局指标（使用各自最优阈值组合？不推荐！）
    #     # 注意：不能简单拼接 per-label 最优预测，因为阈值不同，无法构成合法的多标签输出
    #     # 所以通常只报告 per-label 指标，或另选一个全局阈值用于 global metrics

    #     # 如果你仍想报告一个全局指标（比如用 0.5 阈值），可以加：
    #     y_pred_global = (self.y_prob > 0.5).astype(int)
    #     global_hamming = hamming_loss(self.y_true, y_pred_global)
    #     global_exact = (self.y_true == y_pred_global).all(axis=1).mean()
    #     global_jaccard = jaccard_score(self.y_true, y_pred_global, average='samples')

    #     self.logger.info(f"\n=== GLOBAL METRICS (fixed threshold=0.5) ===")
    #     self.logger.info(f"Hamming Loss: {global_hamming:.4f}")
    #     self.logger.info(f"Exact Match Ratio: {global_exact:.4f}")
    #     self.logger.info(f"Jaccard (samples): {global_jaccard:.4f}")
    ...
    #             auc = roc_auc_score(y_true=self.y_true[:, i],y_score=self.y_prob[:, i])
    #             if auc > bestscore:
    #                 thresh = threshold
    #                 bestscore = auc
    ...
    def value(self):
        '''
        计算指标得分：每个指标独立搜索其最优阈值（0.01~0.99，步长0.01）
        '''
        # 使用更细粒度的阈值（避免因步长过大导致极端值）
        ...
        for i, label in self.id2label.items():
            ...
            try:
                ...
                auc = float('nan')  # 全正/全负时 AUC 无定义
            best_score.append(auc)
            best_threshold.append(float('nan'))  # AUC 无对应阈值

            # --- Hamming Loss: 报告最小值（而非平均）---
            best_hamming = float('inf')
            best_hamming_thresh = 0.5
            for th in thresholds:
                y_pred_i = (self.y_prob[:, i] > th).astype(int)
                h_loss = hamming_loss(y_true=self.y_true[:, i], y_pred=y_pred_i)
                if h_loss < best_hamming:
                    best_hamming = h_loss
                    best_hamming_thresh = th
            best_score.append(best_hamming)
            best_threshold.append(best_hamming_thresh)

            # --- Hamming Score = 1 - Hamming Loss ---
            best_score.append(1 - best_hamming)
            best_threshold.append(best_hamming_thresh)  # 与 Hamming Loss 共享阈值

            # --- Precision ---
            best_p, best_p_th = 0, 0.5
            for th in thresholds:
                y_pred_i = (self.y_prob[:, i] > th).astype(int)
                p = precision_score(y_true=self.y_true[:, i], y_pred=y_pred_i, average=self.average, zero_division=0)
                if p > best_p:
                    best_p = p
                    best_p_th = th
            best_score.append(best_p)
            best_threshold.append(best_p_th)

            # --- Recall ---
            best_r, best_r_th = 0, 0.5
            for th in thresholds:
                y_pred_i = (self.y_prob[:, i] > th).astype(int)
                r = recall_score(y_true=self.y_true[:, i], y_pred=y_pred_i, average=self.average, zero_division=0)
                if r > best_r:
                    best_r = r
                    best_r_th = th
            best_score.append(best_r)
            best_threshold.append(best_r_th)

            # --- F1 ---
            best_f1, best_f1_th = 0, 0.5
            for th in thresholds:
                y_pred_i = (self.y_prob[:, i] > th).astype(int)
                f1 = f1_score(y_true=self.y_true[:, i], y_pred=y_pred_i, average=self.average, zero_division=0)
                if f1 > best_f1:
                    best_f1 = f1
                    best_f1_th = th
            best_score.append(best_f1)
            best_threshold.append(best_f1_th)

            # --- 输出 ---
            log_msg = (
                f"Label: {label} | "
                f"AUC: {best_score[0]:.4f}, "
                f"HammingLoss: {best_score[1]:.4f} "
                f"HammingScore: {best_score[2]:.4f}, "
                f"Precision: {best_score[3]:.4f} "
                f"Recall: {best_score[4]:.4f} "
                f"F1: {best_score[5]:.4f} "
            )
