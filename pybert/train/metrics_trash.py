...
class ClassReport(Metric):
    ...
    def __init__(self):
        ...
        # 修改为真实标签：['Usa', 'Sup', 'Rel', 'Per', 'Mis']
        # self.target_names = ['Usa', 'Sup', 'Rel', 'Per', 'Mis']
    ...
    def __call__(self, logits, target):
        # 注意：如果是多标签分类，这里用 sigmoid + 阈值；如果是单标签，应该用 argmax
        # 根据你原代码逻辑，这里假设是多标签（因为用了 sigmoid）
        self.y_pred = (logits.sigmoid() > 0.5).int().cpu().numpy()
        self.y_true = target.cpu().numpy()
...
# class ClassReport(Metric):
#     '''
#     Classification report for 5-class NFR classification including 'Mis'
#     Assumes input logits and target are for 4 base classes: ['Usa', 'Sup', 'Rel', 'Per']
#     The 5th class 'Mis' is derived as: Mis = 1 if all 4 labels are 0, else 0.
#     '''
#     def __init__(self):
#         super(ClassReport, self).__init__()
#         self.target_names = ['Usa', 'Sup', 'Rel', 'Per', 'Mis']
...
#     def __call__(self, logits, target):
#         """
#         logits: [B, 4] raw model outputs (before sigmoid)
#         target: [B, 4] ground truth binary labels
#         """
#         # Get predictions for the 4 base classes
#         y_pred_4 = (logits.sigmoid() > 0.5).int().cpu().numpy()  # [B, 4]
#         y_true_4 = target.cpu().numpy()                           # [B, 4]

#         # Derive Mis label: 1 if all 4 are 0, else 0
#         pred_mis = (y_pred_4.sum(axis=1) == 0).astype(int).reshape(-1, 1)  # [B, 1]
#         true_mis = (y_true_4.sum(axis=1) == 0).astype(int).reshape(-1, 1)  # [B, 1]

#         # Concatenate to form 5-class representation
#         self.y_pred = np.concatenate([y_pred_4, pred_mis], axis=1)  # [B, 5]
#         self.y_true = np.concatenate([y_true_4, true_mis], axis=1)  # [B, 5]

#     def value(self):
#         '''
#         Compute per-class classification report including 'Mis'
#         '''
#         score = classification_report(
#             y_true=self.y_true,
#             y_pred=self.y_pred,
#             target_names=self.target_names,
#             zero_division=0  # Avoid warnings when no samples/predictions for a class
#         )
#         return score
